update_banner_slot_inventory_on_order-- 상하반기 재고관리 최종 수정 스크립트
-- 스키마와 함수를 연관지어서 완전히 해결
...
-- 2. 상하반기 구분이 명확한 재고 관리 함수들

-- 주문 시 특정 상하반기의 재고만 감소
CREATE OR REPLACE FUNCTION update_banner_slot_inventory_on_order()
RETURNS TRIGGER AS $$update_banner_slot_inventory_on_order
DECLARE
  period_id UUID;
  panel_record RECORD;
  target_period TEXT;
BEGIN
  ...
  IF period_id IS NOT NULL THEN
    ...
    RAISE NOTICE '재고 업데이트 완료: panel_info_id=%, period_id=%, period=%, quantity=%', 
      NEW.panel_info_id, period_id, target_period, NEW.slot_order_quantity;
  ELSE
    -- 기간을 찾지 못한 경우 로그 출력 (디버깅용)
    RAISE WARNING '기간을 찾을 수 없음: panel_info_id=%, display_start_date=%', 
      NEW.panel_info_id, NEW.display_start_date;
  END IF;
...
BEGIN
  ...
  IF period_id IS NOT NULL THEN
    ...
    RAISE NOTICE '재고 복구 완료: panel_info_id=%, period_id=%, period=%, quantity=%', 
...
-- 재고 부족 시 주문 방지 (상하반기 구분) - 문법 오류 수정
...
BEGIN
    ...
    IF EXISTS (
        ...
        LOOP
            -- banner_slot_inventory 생성 (상하반기별 초기 재고 설정)
            INSERT INTO banner_slot_inventory (
                panel_info_id,
                region_gu_display_period_id,
                total_slots,
...
-- 4. 기존 재고 데이터 정리 및 재생성
-- 상하반기 구분이 명확하지 않은 기존 재고 데이터 정리
...
-- 기간별 재고가 없는 패널들에 대해 상하반기별 재고 생성
INSERT INTO banner_slot_inventory (
  panel_info_id,
  region_gu_display_period_id,
  total_slots,
...
SELECT 
  ...
  CASE 
    ...
    WHEN rgdp.period = 'second_half' THEN 
...
SELECT 
  ...
  COUNT(CASE WHEN bsi.available_slots > 0 AND bsi.available_slots <= bsi.total_slots * 0.2 THEN 1 END) as low_stock_panels
FROM banner_slot_inventory bsi
JOIN panel_info pi ON pi.id = bsi.panel_info_id
JOIN region_gu rgu ON rgu.id = pi.region_gu_id
JOIN region_gu_display_periods rgdp ON rgdp.id = bsi.region_gu_display_period_id
...
SELECT 
  ...
  CASE 
    ...
    ELSE 
      pi.max_banner
  END as expected_available_slots
FROM panel_info pi
JOIN region_gu rgu ON rgu.id = pi.region_gu_id
JOIN region_gu_display_periods rgdp ON rgdp.region_gu_id = rgu.id 
  AND rgdp.display_type_id = pi.display_type_id
LEFT JOIN banner_slot_inventory bsi ON bsi.panel_info_id = pi.id 
  AND bsi.region_gu_display_period_id = rgdp.id
WHERE pi.display_type_id = (SELECT id FROM display_types WHERE name = 'banner_display')
  AND pi.panel_status = 'active'
ORDER BY rgu.name, rgdp.year_month, rgdp.period, pi.nickname;

-- 완료 메시지
SELECT '상하반기 재고관리 최종 수정 완료' as status; 