-- LED 전자게시대 캐시 테이블에 panel_status 컬럼 추가 및 수정

-- 1. 캐시 테이블에 panel_status 컬럼 추가
DO $$
BEGIN
  -- panel_status 컬럼 추가
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_name = 'led_display_cache' AND column_name = 'panel_status') THEN
    ALTER TABLE led_display_cache ADD COLUMN panel_status VARCHAR DEFAULT 'active';
  END IF;
END $$;
...
BEGIN
  ...
  RAISE NOTICE 'LED display cache updated successfully with panel_status';
END;
$$ LANGUAGE plpgsql;
