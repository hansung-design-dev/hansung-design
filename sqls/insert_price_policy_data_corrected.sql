...
SELECT 
  ...
  COUNT(*) as 게시대개수,
  '상업용(default), 재사용(re_order), 자체제작(self_install) 3가지 가격정책 적용' as 비고
FROM banner_slot_info bsi
JOIN panel_info pi ON pi.id = bsi.panel_info_id
JOIN region_gu rgu ON rgu.id = pi.region_gu_id
WHERE rgu.name = '관악구' 
...
SELECT 
  ...
  pi.panel_type as 패널타입,
  COUNT(*) as 게시대개수,
  '상업용(default), 행정용(public_institution) 2가지 가격정책 적용' as 비고
FROM banner_slot_info bsi
JOIN panel_info pi ON pi.id = bsi.panel_info_id
JOIN region_gu rgu ON rgu.id = pi.region_gu_id
WHERE rgu.name = '송파구' 
...
-- 서대문구 판넬 행정용 (panel 타입, slot_number = 1 기준)
INSERT INTO banner_slot_price_policy (
  id,
  banner_slot_info_id,
  price_usage_type,
...
-- 서대문구 반자동 행정용 (semi_auto 타입, slot_number = 1 기준)
INSERT INTO banner_slot_price_policy (
  id,
  banner_slot_info_id,
  price_usage_type,
...
-- 서대문구 가격정책별 게시대 개수 확인
SELECT 
  '서대문구' as 행정구,
  pi.panel_type as 패널타입,
  COUNT(*) as 게시대개수,
  CASE 
    WHEN pi.panel_type = 'panel' THEN '상업용(default), 행정용(public_institution) 2가지 가격정책 적용'
    WHEN pi.panel_type = 'semi_auto' THEN '행정용(public_institution) 1가지 가격정책 적용'
  END as 비고
FROM banner_slot_info bsi
JOIN panel_info pi ON pi.id = bsi.panel_info_id
JOIN region_gu rgu ON rgu.id = pi.region_gu_id
WHERE rgu.name = '서대문구' 
  AND pi.panel_type IN ('panel', 'semi_auto')
  AND bsi.slot_number = 1
  AND pi.display_type_id = (SELECT id FROM display_types WHERE name = 'banner_display')
  AND pi.panel_status = 'active'
  AND rgu.is_active = true
...
-- 서대문구 실제 생성된 가격정책 로우 개수 확인
SELECT 
  '서대문구' as 행정구,
  pi.panel_type as 패널타입,
  bsp.price_usage_type as 가격정책타입,
  COUNT(bsp.id) as 생성된로우개수,
  CASE 
...
WHERE rgu.name = '서대문구' 
  ...
  AND pi.panel_status = 'active'
  AND rgu.is_active = true
GROUP BY pi.panel_type, bsp.price_usage_type
ORDER BY pi.panel_type, bsp.price_usage_type;

-------------------
-- 용산구
-------------------
-- 용산구 판넬 상업용 (panel, with_lighting, no_lighting 모두 포함, slot_number = 1 기준)
INSERT INTO banner_slot_price_policy (
  id,
  banner_slot_info_id,
  price_usage_type,
...
-- 용산구 판넬 행정용 (panel, with_lighting, no_lighting 모두 포함, slot_number = 1 기준)
INSERT INTO banner_slot_price_policy (
  id,
  banner_slot_info_id,
  price_usage_type,
...
-- 용산구 반자동 상업용 (semi_auto 타입, slot_number = 1 기준)
INSERT INTO banner_slot_price_policy (
  id,
  banner_slot_info_id,
  price_usage_type,
...
-- 용산구 반자동 행정용 (semi_auto 타입, slot_number = 1 기준)
INSERT INTO banner_slot_price_policy (
  id,
  banner_slot_info_id,
  price_usage_type,
...
SELECT 
  ...
  COUNT(*) as 게시대개수,
  CASE 
    WHEN pi.panel_type IN ('panel', 'with_lighting', 'no_lighting') THEN '상업용(default), 행정용(public_institution) 2가지 가격정책 적용'
    WHEN pi.panel_type = 'semi_auto' THEN '상업용(default), 행정용(public_institution) 2가지 가격정책 적용'
  END as 비고
FROM banner_slot_info bsi
JOIN panel_info pi ON pi.id = bsi.panel_info_id
JOIN region_gu rgu ON rgu.id = pi.region_gu_id
WHERE rgu.name = '용산구' 
  AND pi.panel_type IN ('panel', 'with_lighting', 'no_lighting', 'semi_auto')
  AND bsi.slot_number = 1
  AND pi.display_type_id = (SELECT id FROM display_types WHERE name = 'banner_display')
  AND pi.panel_status = 'active'
  AND rgu.is_active = true
...
-- 마포구
-------------------
-- 마포구 연립형 상업용 (multi_panel 타입, slot_number = 1 기준)
INSERT INTO banner_slot_price_policy (
  id,
  banner_slot_info_id,
  price_usage_type,
...
-- 마포구 연립형 행정용 (multi_panel 타입, slot_number = 1 기준)
INSERT INTO banner_slot_price_policy (
  id,
  banner_slot_info_id,
  price_usage_type,
...
-- 마포구 저단형 상업용 (lower_panel 타입, slot_number = 1 기준)
INSERT INTO banner_slot_price_policy (
  id,
  banner_slot_info_id,
  price_usage_type,
...
-- 마포구 저단형 행정용 (lower_panel 타입, slot_number = 1 기준)
INSERT INTO banner_slot_price_policy (
  id,
  banner_slot_info_id,
  price_usage_type,
...
-- 마포구 저단형 행정용자리대여 (0원) (lower_panel 타입, slot_number = 1 기준)
INSERT INTO banner_slot_price_policy (
  id,
  banner_slot_info_id,
  price_usage_type,
...
SELECT 
  ...
  COUNT(*) as 게시대개수,
  CASE 
    WHEN pi.panel_type = 'multi_panel' THEN '상업용(default), 행정용(public_institution) 2가지 가격정책 적용'
    WHEN pi.panel_type = 'lower_panel' THEN '상업용(default), 행정용(public_institution), 관리자할인(reduction_by_admin) 3가지 가격정책 적용'
  END as 비고
FROM banner_slot_info bsi
JOIN panel_info pi ON pi.id = bsi.panel_info_id
JOIN region_gu rgu ON rgu.id = pi.region_gu_id
WHERE rgu.name = '마포구' 
  AND pi.panel_type IN ('multi_panel', 'lower_panel')
  AND bsi.slot_number = 1
  AND pi.display_type_id = (SELECT id FROM display_types WHERE name = 'banner_display')
  AND pi.panel_status = 'active'
  AND rgu.is_active = true
...
LEFT JOIN panel_info pi ON pi.region_gu_id = rgu.id 
  AND pi.display_type_id = (SELECT id FROM display_types WHERE name = 'banner_display')
  AND pi.panel_status = 'active'
LEFT JOIN banner_slot_info bsi ON bsi.panel_info_id = pi.id AND bsi.slot_number = 1
