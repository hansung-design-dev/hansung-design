-- ⚠️ 긴급 수정: update_banner_slot_inventory_on_order 함수
-- Supabase에서 이 함수만 실행하세요!
...
BEGIN
  ...
  -- ⚠️ NEW.banner_slot_id를 직접 참조하지 않음!
...
-- 확인: 함수가 올바르게 수정되었는지 확인
SELECT 
  proname,
  CASE 
    WHEN prosrc LIKE '%NEW.banner_slot_id%' THEN '❌ 여전히 NEW.banner_slot_id 참조 있음!'
