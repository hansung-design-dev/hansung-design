...
BEGIN
  ...
  LEFT JOIN panels p ON rg.id = p.region_gu_id 
    AND p.display_type_id = '3119f6ed-81e4-4d62-b785-6a33bc7928f9'  -- LED display type ID
  ...
  GROUP BY rg.id, rg.name, rg.code, rg.logo_image_url, rg.phone_number, 
           rg.display_order, ba.bank_name, ba.account_number, ba.depositor
...
-- LED 캐시 테이블 자동 업데이트 트리거 함수
CREATE OR REPLACE FUNCTION trigger_update_led_display_cache()
RETURNS trigger AS $$
BEGIN
  -- LED 관련 테이블이 변경될 때 캐시 업데이트
...
-- LED 관련 테이블에 트리거 추가
DROP TRIGGER IF EXISTS trigger_led_display_cache_update ON panels;
CREATE TRIGGER trigger_led_display_cache_update
  ...
  EXECUTE FUNCTION trigger_update_led_display_cache();

DROP TRIGGER IF EXISTS trigger_led_display_cache_update_price ON led_display_price_policy;
CREATE TRIGGER trigger_led_display_cache_update_price
  AFTER INSERT OR UPDATE OR DELETE ON led_display_price_policy
  FOR EACH ROW
  EXECUTE FUNCTION trigger_update_led_display_cache();

DROP TRIGGER IF EXISTS trigger_led_display_cache_update_period ON region_gu_display_periods;
CREATE TRIGGER trigger_led_display_cache_update_period
  AFTER INSERT OR UPDATE OR DELETE ON region_gu_display_periods
  FOR EACH ROW
  WHEN (NEW.display_type_id = '3119f6ed-81e4-4d62-b785-6a33bc7928f9' OR 
        OLD.display_type_id = '3119f6ed-81e4-4d62-b785-6a33bc7928f9')
  EXECUTE FUNCTION trigger_update_led_display_cache();
